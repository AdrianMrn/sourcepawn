...
stock void RegisterCommands()
{
	...
	RegConsoleCmd("tk", OnCommandTakeKnife);
	...
	RegConsoleCmd("takeweapon", OnCommandTakeWeapon);
	RegConsoleCmd("tw", OnCommandTakeWeapon);
	
	RegConsoleCmd("takepistol", OnCommandTakePistol);
	RegConsoleCmd("tp", OnCommandTakePistol);
}
...
public Action OnCommandTakeWeapon(int client, int args)
{
	if (!IsClientInGame(client))
		return Plugin_Handled;
	...
	if (!IsPlayerAlive(client))
	{
		ReplyToCommand(client, "[SM] You need to be alive to use !takeweapon.");
		return Plugin_Handled;
	}
	...
	if (StrEqual("", target))
	{
		ReplyToCommand(client, "[SM] Usage: !takeweapon <player>.");
		return Plugin_Handled;
	}
	...
	if (targets[0] == client)
	{
		ReplyToCommand(client, "[SM] You cant swap your own weapon!");
		return Plugin_Handled;
	}
	...
	if (g_bBlockTakeKnife[targets[0]])
	{
		ReplyToCommand(client, "[SM] Target has blocked players from taking their weapon!");
		return Plugin_Handled;
	}
	
	GivePlayerWeapon(targets[0], client, client);
	g_iTakeKnifeUserIds[client] = GetClientUserId(targets[0]);
	return Plugin_Handled;
}

public Action OnCommandTakePistol(int client, int args)
{
	if (!IsClientInGame(client))
		return Plugin_Handled;
	...
	if (!IsPlayerAlive(client))
	{
		ReplyToCommand(client, "[SM] You need to be alive to use !takepistol.");
		return Plugin_Handled;
	}
	...
	if (StrEqual("", target))
	{
		ReplyToCommand(client, "[SM] Usage: !takepistol <player>.");
		return Plugin_Handled;
	}
	...
	if (targets[0] == client)
	{
		ReplyToCommand(client, "[SM] You cant swap your own pistol!");
		return Plugin_Handled;
	}
	...
	if (g_bBlockTakeKnife[targets[0]])
	{
		ReplyToCommand(client, "[SM] Target has blocked players from taking their pistol!");
		return Plugin_Handled;
	}
	
	GivePlayerPistol(targets[0], client, client);
	g_iTakeKnifeUserIds[client] = GetClientUserId(targets[0]);
	return Plugin_Handled;
}
...
stock Action GivePlayerPistol(int client, int target, int commandclient, bool displaymessage=true)
{
	if (!IsClientInGame(client))
		return Plugin_Stop;
	...
	// clsName now contains the weapon_<type> 
	
	if (weapon == -1)
	{
		ReplyToCommand(commandclient, "[SM] Pistol not found!");
		return Plugin_Stop;
	}
	...
	int newweapon = GivePlayerItem(client, clsName);
	//g_bBlockItemEquip[client] = false; // paintkits fix
	//if (newweapon == -1)
	//{
		//ReplyToCommand(commandclient, "[SM] Unable to spawn your weapon!");
		//return Plugin_Stop;
	//}
	
	//int weaponindex = GetEntProp(newweapon, Prop_Send, "m_iItemDefinitionIndex");
	...
	bool equiptargetnewweapon = false;
	int targetactiveweapon = GetEntPropEnt(target, Prop_Send, "m_hActiveWeapon");
	//bool equiptaser = false;
	int targetweapon = GetPlayerWeaponSlot(target, CS_SLOT_SECONDARY);
	
	while (targetweapon != -1)
	{
		// check for taser
		//if (GetEntProp(targetweapon, Prop_Send, "m_iItemDefinitionIndex") == 31)
		//	equiptaser = true;
		//else
		//{
		if (targetactiveweapon == targetweapon)
			equiptargetnewweapon = true;
		//}
		
		RemovePlayerItem(target, targetweapon);
		AcceptEntityInput(targetweapon, "Kill");
		targetweapon = GetPlayerWeaponSlot(target, CS_SLOT_SECONDARY);
	}
	
	float targetvec[3];
	GetEntPropVector(target, Prop_Send, "m_vecOrigin", targetvec);
	TeleportEntity(newweapon, targetvec, NULL_VECTOR, NULL_VECTOR);
	
	EquipPlayerWeapon(target, newweapon);
	if (equiptargetnewweapon)
		CreateTimer(0.01, OnEquipPlayerPistolRequired, GetClientUserId(target));
		
	//if (equiptaser)
	//	GivePlayerItem(target, "weapon_taser");
	
	PrintToChat(target, "\x01[SM] You received a pistol from \x04%N", client);
	
	if (displaymessage)
		PrintToChat(client, "\x01[SM] You gave your pistol to \x04%N", target);
	return Plugin_Continue;
}

public Action OnEquipPlayerPistolRequired(Handle tmr, any userid)
{
	int client = GetClientOfUserId(userid);
	if (client == 0)
		return Plugin_Continue;

	int targetweapon = GetPlayerWeaponSlot(client, CS_SLOT_SECONDARY);
	if (targetweapon == -1)
		return Plugin_Continue;
		
	SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", targetweapon);
	return Plugin_Continue;
}

stock Action GivePlayerWeapon(int client, int target, int commandclient, bool displaymessage=true)
{
	if (!IsClientInGame(client))
		return Plugin_Stop;
	...
	if (weapon == -1)
	{
		ReplyToCommand(commandclient, "[SM] Weapon not found!");
		return Plugin_Stop;
	}
	...
	int targetactiveweapon = GetEntPropEnt(target, Prop_Send, "m_hActiveWeapon");
	//bool equiptaser = false;
	int targetweapon = GetPlayerWeaponSlot(target, CS_SLOT_PRIMARY);
	...
	while (targetweapon != -1)
	{
		...
		RemovePlayerItem(target, targetweapon);
		AcceptEntityInput(targetweapon, "Kill");
		targetweapon = GetPlayerWeaponSlot(target, CS_SLOT_PRIMARY);
	}
	...
	EquipPlayerWeapon(target, newweapon);
	if (equiptargetnewweapon)
		CreateTimer(0.01, OnEquipPlayerWeaponRequired, GetClientUserId(target));
		
	//if (equiptaser)
	//	GivePlayerItem(target, "weapon_taser");
	
	PrintToChat(target, "\x01[SM] You received a weapon from \x04%N", client);
	
	if (displaymessage)
		PrintToChat(client, "\x01[SM] You gave your weapon to \x04%N", target);
	return Plugin_Continue;
}

public Action OnEquipPlayerWeaponRequired(Handle tmr, any userid)
{
	int client = GetClientOfUserId(userid);
	if (client == 0)
		return Plugin_Continue;

	int targetweapon = GetPlayerWeaponSlot(client, CS_SLOT_PRIMARY);
	if (targetweapon == -1)
		return Plugin_Continue;
